...
void PrimaryGeneratorAction::Cobalt60(float ConstituentPos[],float ConstituentDir[],G4Event* anEvent,G4ParticleGun*PG,char misc,char gamma){
...
void PrimaryGeneratorAction::CircularPlateSource(G4int tot,G4int num,G4double Radius,G4double PSPos[],void (*Process)(float[],float[],G4Event*,G4ParticleGun*,char,char),G4Event* anEvent,char misc,char gamma){
	...
	for(int i = 0; i < tot; i++){
		...
		Process(ConstituentPos,ConstituentDir,anEvent,fParticleSource,misc,gamma); // num us encoded in the Process
	}
}
