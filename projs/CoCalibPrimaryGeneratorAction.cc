...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
	...
	if(misc == 'b'){
		...
		fParticleSource->SetParticleDefinition(G4Gamma::Definition());
	} else if (misc == 't'){ // Thorium 232

		fParticleSource->SetParticleEnergy(2615*keV);
		fParticleSource->SetParticleDefinition(G4Gamma::Definition());
	} else { // muons
	...
	fParticleSource->SetParticlePosition(G4ThreeVector(0.0*mm,0.0*mm,-20.0*cm));
	//fParticleSource->SetParticlePosition(G4ThreeVector(0.0*mm,0.0*mm,-65.0*mm));
	fParticleSource->SetParticleMomentumDirection(G4ThreeVector(0.0, 0.0, 1.0));
	...
	G4double CirclePSPos[3] = {0,0,-0.2};
